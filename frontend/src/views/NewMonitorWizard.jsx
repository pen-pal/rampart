...
import {
  SyntheticSteps, blankSynthStep, uiToConfigSteps, parseHeaders, headersToText,
...
export default function NewMonitorWizard() {
  ...
  // parseHeaders / headersToText are shared with the edit modal — imported
  // from ../components/SyntheticSteps.jsx.
  ...
  const buildPayload = () => {
    ...
    // Synthetic-transaction step sequence → config.steps (shared converter,
    // identical to the edit modal's). Drops blank rows + empty optional pieces.
    if (fields.synthetic) {
      const synth = uiToConfigSteps(synthSteps);

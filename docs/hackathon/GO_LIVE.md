...
> the leading `v`). Pin a tag — this runbook uses **`0.157.5`** (the current
...
3. Pin the image tag in the compose file (it defaults to `:latest`) and bring it
   ...
   ```bash
   # in deploy/compose.aws.yaml, set:  image: ghcr.io/pen-pal/rampart:0.157.5
...
docker run --rm \
  ...
  ghcr.io/pen-pal/rampart:0.157.5 rampart-api seed-demo
```

...
| `rand`                | 0.8     | 0.10        | **Locked behind the RustCrypto stack.** `rand` 0.10 ships `rand_core` 0.9; the elliptic-curve crate (used by `p256` / `aes-gcm` for web push) still requires `rand_core` 0.6's `CryptoRngCore` trait, so handing it a `ThreadRng` from `rand 0.10` fails the trait bound. Bumping `rand` therefore requires bumping `elliptic-curve` + `p256` + `aes-gcm` + `sha2` + `hmac` + `hkdf` in lockstep — the next row below. Method renames (`thread_rng()` → `rng()`, `gen_range` → `random_range`, `use rand::Rng;` → `use rand::RngExt;`) are mechanical but only meaningful as part of that batch. |

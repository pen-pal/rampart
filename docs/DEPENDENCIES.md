...
| :-------------------- | :------ | :---------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| RustCrypto bundle (`rand`, `sha2`, `hmac`, `hkdf`, `p256`, `aes-gcm`, `md-5`, `sha1`) | 0.8 / 0.10 / 0.12 / 0.12 / 0.13 / 0.10 / 0.10 / 0.10 | 0.10 / 0.11 / 0.13 / 0.13 / 0.14 / 0.11 / 0.11 / 0.11 | **Blocked on the upstream RustCrypto coordinated stable release.** These crates have to bump together — `rand` 0.10 ships `rand_core` 0.9, the elliptic-curve / aead / digest traits in the rest of the bundle still want `rand_core` 0.6 in their current stable revs, so handing `SecretKey::random(&mut rand::rng())` a `ThreadRng` from `rand 0.10` fails the trait bound. **Re-attempted 2026-06-04 + a second time after**; the upstream coordinated rev still exists only on the RC channel (e.g. `aes-gcm 0.11.0-rc.4`, `p256 0.14.0-pre`). Pinning to RC revs is not appropriate for a published security surface — wait for the matched stables. Mechanical changes when the rev lands: `thread_rng()` → `rng()`, `gen_range` → `random_range`, `use rand::Rng;` → `use rand::RngExt;`, and the `GenericArray::{as_slice,from_slice}` → `hybrid-array::Array` migration in `rampart-notifier/src/channels/webpush_crypto.rs`. |
| `rumqttc`             | 0.24    | 0.25        | **Rejected, not just deferred.** 0.25 unconditionally adds `aws-lc-rs` + `cmake` to the build graph, which contradicts the pure-Rust crypto invariant the project advertises (see README "Why Rampart" + `docs/DESIGN-ORIGINAL.md`). Re-attempted today — same blocker holds; `cargo tree -i aws-lc-rs` reports the dep coming through `rustls v0.23 → bollard + hyper-rustls + lettre + mongodb + redis + sqlx-core + rumqttc` once 0.25 is selected. Will revisit only if upstream restores a featureless rustls path. |

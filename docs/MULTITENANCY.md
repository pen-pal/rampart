...
| 4 | **Org CRUD + switcher UI + write-stamping + OIDC→org + per-org RBAC** | Owner green-lit P4→P5→P6 (2026-06-18). Sliced 4a-4g: **4a per-INSERT org-stamping ✅ (v0.130.0)** — every tenant-root create stamps `org_id` explicitly (param threaded from `OrgContext`; token-less ingest stamps Default w/ `// P5` marker; `/v1/metrics/ingest` stamps caller org; `delivery_log` COALESCE-from-notification; UNNEST via ARRAY_FILL); behaviour-identical single-org; test `org_write_stamping.rs`. **4b** db primitives + the users.role↔Default-membership mirror; **4c** `/v1/orgs` CRUD + members (any user creates, creator=Admin, atomic); **4d** switcher (set active_org_id) + enrich `/v1/auth/me`; **4e** per-org RBAC (OrgContext.role from org_members; stale/non-member active_org → Default fallback); **4f** OIDC→org (configurable `RAMPART_OIDC_ORG_CLAIM`, default groups→slug; unset = today's behaviour); **4g** frontend switcher + Organizations admin page. No enforcement flip (that's P6). | in progress |

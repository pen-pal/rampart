...
| 3 | **Read filtering (management API)** | `WHERE org_id = $ctx` on every management query + ID-set endpoint validation; scheduler/notifier given explicit unscoped/same-org helpers. Rolling out per-domain: **3a monitors ✅ (v0.109.0)**, **3b alert-rules + silences ✅ (v0.110.0)**, **3c SLOs + on-call ✅ (v0.111.0)**, **3d notification channels ✅ (v0.112.0)**; rest of alerting (escalations/detection/delivery-log) + status-pages / infra-credentials / telemetry / cross-cutting follow. Secure-by-default: scoped fns keep the plain name, system callers use explicit `*_all`/`*_unscoped` siblings. | in progress |

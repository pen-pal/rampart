...
| 6 | **Flip enforcement** | `SET NOT NULL` + drop the column DEFAULT (0112), per-org uniqueness for `tags.name`/`notification_templates.name` (0113), bearer org from `api_keys.org_id`; then (behind `RAMPART_MULTI_ORG`, held for explicit OK) tighten the silent-Default cookie/ingest fallbacks; Postgres RLS as defense-in-depth is **deferred** (needs a dual-pool + request-tx refactor — app-level `WHERE org_id` already ships + is tested). | 🔄 in progress |

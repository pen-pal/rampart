...
| 6 | **Flip enforcement** | Safe core ✅ (v0.145.0): `SET NOT NULL` + drop the column DEFAULT (0112), per-org uniqueness for `tags.name`/`notification_templates.name` (0113), bearer org from `api_keys.org_id`. Ingest enforcement ✅ (v0.146.0): behind `RAMPART_MULTI_ORG` (opt-in, default off) an ingest key-miss is `Unauthorized` instead of falling back to Default. The cookie path is intentionally **not** hard-denied on a stale active-org (it already resolves only to orgs the caller is a member of, and a global 403 would lock the user out of the org-switch endpoint too) — it keeps the graceful Default fallback. Postgres RLS as defense-in-depth is **dormant-shipped** behind `RAMPART_RLS` (default off): migration 0114 adds the `rampart_app` role + grants, 0115 defines the org-isolation policies, and the pool binds the per-request org via a `SET ROLE` + `app.current_org` GUC hook (system loops bypass as the owner). **No table is `ENABLE`/`FORCE`d yet** — the policies are inert until an operator opts in (see "Defense-in-depth: Row-Level Security" below). | 🔄 ingest enforcement shipped; RLS dormant-shipped (`RAMPART_RLS`) |
...
**The policies ship DORMANT.** No migration runs `ALTER TABLE … ENABLE/FORCE
ROW LEVEL SECURITY`, so even with `RAMPART_RLS=1` nothing is *enforced* yet —
the flag only changes which role/GUC each connection carries. Enforcement is a
deliberate, separate operator step. Before flipping `RAMPART_RLS` on, the
operator **must** grant the login role `BYPASSRLS` (`ALTER ROLE <db_user>
BYPASSRLS;`) so the system loops keep working — see `docs/SETUP.md` production
checklist.

...
│       ├── rampart-checker    — probe runners (27 kinds)
...
| Area                | Status                                              |
| ---                 | ---                                                 |
| **Probes**          | 27 / 27 implemented — HTTP / keyword / JSON, TCP, ping (ICMP), DNS, push, TLS cert, domain expiry, Postgres / MySQL / MSSQL / Redis / MongoDB / Memcached, gRPC (health.v1), MQTT, Docker, Steam (A2S), Kafka (ApiVersions), RADIUS, headless-browser keyword (via an external renderer), SSH / SMTP / IMAP / FTP / POP3 (banner check) |
...
| **Dependencies**    | Monitor dependencies with upstream alert suppression (a down parent silences dependents so one root cause doesn't paging-storm), cycle-guarded |

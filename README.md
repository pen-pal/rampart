...
| Area                | Status                                              |
| ---                 | ---                                                 |
| **Probes**          | 28 / 28 implemented — HTTP / keyword / JSON, TCP, ping (ICMP), DNS, push, TLS cert, domain expiry, Postgres / MySQL / MSSQL / Redis / MongoDB / Memcached, NTP (SNTPv4), gRPC (health.v1), MQTT, Docker, Steam (A2S), Kafka (ApiVersions), RADIUS, headless-browser keyword (via an external renderer), SSH / SMTP / IMAP / FTP / POP3 (banner check) |

...
- **AMQP probe (`MonitorKind::Amqp`)** — connects to a RabbitMQ-compatible broker over `amqp://`, completes the AMQP 0-9-1 protocol handshake (version negotiation → SASL PLAIN auth → tune / open) via the `lapin` client, closes cleanly with a goodbye frame. Up means the broker is reachable AND credentials are valid AND the requested vhost exists. `amqps://` (TLS) is a follow-up. Probe count: 32. `rampart-checker/src/amqp.rs`, migration `0037_amqp.sql`, wizard card with `amqp://guest:guest@rabbitmq.internal:5672/%2F` placeholder.

...
async fn recent_active_first_then_newest(pool: PgPool) {
    ...
    let mk = |title: &str, style: &str| -> NewIncident {
        serde_json::from_value(
            serde_json::json!({ "title": title, "content": "x", "style": style }),
        )
        .unwrap()
    };
    let outage = incidents::create(&pool, sp.id, None, mk("DB outage", "danger"))
        .await
        .unwrap();
    let degraded = incidents::create(&pool, sp.id, None, mk("Slow", "warning"))
        .await
        .unwrap();
    // Resolve the degraded one → inactive; the active outage should sort first.
    incidents::resolve(&pool, degraded.id, OffsetDateTime::now_utc())
        .await
        .unwrap();
    ...
    assert_eq!(
        recent[0].id, outage.id,
        "active incident sorts before resolved"
    );
    assert!(recent[0].active && !recent[1].active);
    assert_eq!(
        recent[0].style,
        rampart_core::incident::IncidentStyle::Danger
    );
}

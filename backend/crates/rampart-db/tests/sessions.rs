...
async fn make_user(pool: &PgPool, email: &str) -> rampart_core::ids::UserId {
    rampart_db::users::create(pool, NewUser {
        email: email.into(),
        name: None,
...
async fn create_and_read_back(pool: PgPool) {
    ...
    let s = create(&pool, uid, 3600, None, Some("curl/8".into())).await.unwrap();
...
async fn expired_session_is_not_returned(pool: PgPool) {
    ...
    assert!(matches!(err, rampart_db::DbError::NotFound), "expired session should not be returned");
}
...
async fn cleanup_expired_only_drops_old(pool: PgPool) {
    ...
    assert!(dropped >= 1, "expected at least the one expired session deleted");

...
async fn create_round_trips(pool: PgPool) {
    let m = create(&pool, http_monitor("api", "https://api.example.com")).await.unwrap();
...
async fn list_returns_all_in_recency_order(pool: PgPool) {
    let _a = create(&pool, http_monitor("a", "https://a.example.com")).await.unwrap();
    let _b = create(&pool, http_monitor("b", "https://b.example.com")).await.unwrap();
    let _c = create(&pool, http_monitor("c", "https://c.example.com")).await.unwrap();
    let ms = list(&pool).await.unwrap();
    assert_eq!(ms.len(), 3);
}
...
async fn set_active_toggles_field(pool: PgPool) {
    let m = create(&pool, http_monitor("toggle", "https://x.example.com")).await.unwrap();
    assert!(m.active);
...
async fn delete_removes_from_list(pool: PgPool) {
    let m = create(&pool, http_monitor("gone", "https://gone.example.com")).await.unwrap();

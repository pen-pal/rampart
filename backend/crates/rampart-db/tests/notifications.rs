...
async fn create_and_get(pool: PgPool) {
    let n = create(&pool, webhook_channel("ch"), TEST_ORG)
        .await
        .unwrap();
    assert_eq!(n.name, "ch");
...
async fn attach_detach_round_trip(pool: PgPool) {
    let m = monitors::create(&pool, http_monitor("m1"), TEST_ORG)
        .await
        .unwrap();
    let c = create(&pool, webhook_channel("ch"), TEST_ORG)
        .await
        .unwrap();
...
async fn counts_per_monitor_excludes_disabled(pool: PgPool) {
    let m = monitors::create(&pool, http_monitor("counts"), TEST_ORG)
        .await
        .unwrap();
    let c1 = create(&pool, webhook_channel("c1"), TEST_ORG)
        .await
        .unwrap();
    let c2 = create(&pool, webhook_channel("c2"), TEST_ORG)
        .await
        .unwrap();

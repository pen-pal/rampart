...
async fn browser_breakdown_classifies_and_counts(pool: PgPool) {
    ...
    let bd = rum::browser_breakdown(&pool, None, 24, TEST_ORG).await.unwrap();
...
async fn recent_traced_returns_only_traced(pool: PgPool) {
    ...
    let traced = rum::recent_traced(&pool, None, 24, 50, TEST_ORG).await.unwrap();
...
async fn page_samples_drilldown_carries_user(pool: PgPool) {
    ...
    let samples = rum::page_samples(&pool, None, "/checkout", 24, 100, TEST_ORG).await.unwrap();
    ...
    assert_eq!(rum::page_samples(&pool, None, "/other", 24, 100, TEST_ORG).await.unwrap().len(), 1);
}
...
async fn user_breakdown_groups_by_user(pool: PgPool) {
    ...
    let bd = rum::user_breakdown(&pool, None, 24, TEST_ORG).await.unwrap();

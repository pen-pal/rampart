...
async fn browser_breakdown_classifies_and_counts(pool: PgPool) {
    ...
    let bd = rum::browser_breakdown(&pool, None, 24, TEST_ORG)
        .await
        .unwrap();
...
async fn recent_traced_returns_only_traced(pool: PgPool) {
    rum::insert_event(
        &pool,
        &beacon("/checkout", CHROME, Some("trace-abc"), 3000.0),
        TEST_ORG,
    )
    .await
    .unwrap();
    ...
    let traced = rum::recent_traced(&pool, None, 24, 50, TEST_ORG)
        .await
        .unwrap();
...
async fn page_samples_drilldown_carries_user(pool: PgPool) {
    rum::insert_event(
        &pool,
        &beacon_user("/checkout", "alice@example.com"),
        TEST_ORG,
    )
    .await
    .unwrap();
    rum::insert_event(
        &pool,
        &beacon_user("/checkout", "bob@example.com"),
        TEST_ORG,
    )
    .await
    .unwrap();
    rum::insert_event(&pool, &beacon("/other", CHROME, None, 1500.0), TEST_ORG)
        .await
        .unwrap();

    let samples = rum::page_samples(&pool, None, "/checkout", 24, 100, TEST_ORG)
        .await
        .unwrap();
    assert_eq!(samples.len(), 2, "only /checkout loads");
    assert!(samples.iter().all(|s| s.user_id.is_some()));
    let users: Vec<_> = samples
        .iter()
    ...
    assert_eq!(
        rum::page_samples(&pool, None, "/other", 24, 100, TEST_ORG)
            .await
            .unwrap()
            .len(),
        1
...
async fn user_breakdown_groups_by_user(pool: PgPool) {
    rum::insert_event(&pool, &beacon_user("/", "alice@example.com"), TEST_ORG)
        .await
        .unwrap();
    rum::insert_event(
        &pool,
    ...
    .await
    .unwrap();
    rum::insert_event(&pool, &beacon_user("/", "bob@example.com"), TEST_ORG)
        .await
        .unwrap();
    rum::insert_event(&pool, &beacon("/", CHROME, None, 1500.0), TEST_ORG)
        .await
    ...
    let bd = rum::user_breakdown(&pool, None, 24, TEST_ORG)
        .await
        .unwrap();

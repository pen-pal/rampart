...
async fn create_round_trips_all_fields(pool: PgPool) {
    let t = create(&pool, sample("Concise"), def_org()).await.unwrap();
...
async fn duplicate_name_conflicts(pool: PgPool) {
    create(&pool, sample("Dup"), def_org()).await.unwrap();
    let err = create(&pool, sample("Dup"), def_org()).await.unwrap_err();
    assert!(
...
async fn update_changes_body_only(pool: PgPool) {
    let t = create(&pool, sample("Updatable"), def_org()).await.unwrap();
...
async fn update_can_clear_subject_with_explicit_none(pool: PgPool) {
    let t = create(&pool, sample("Sub"), def_org()).await.unwrap();
...
async fn delete_removes_template(pool: PgPool) {
    let t = create(&pool, sample("Gone"), def_org()).await.unwrap();
...
async fn get_render_strings_returns_subject_and_body(pool: PgPool) {
    let t = create(&pool, sample("Render"), def_org()).await.unwrap();

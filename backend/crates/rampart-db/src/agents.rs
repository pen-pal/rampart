...
pub async fn list(pool: &DbPool) -> DbResult<Vec<Agent>> {
...
pub async fn get(pool: &DbPool, id: AgentId) -> DbResult<Agent> {
...
pub async fn update(pool: &DbPool, id: AgentId, patch: UpdateAgent) -> DbResult<Agent> {
...
pub async fn delete(pool: &DbPool, id: AgentId) -> DbResult<()> {
    let result = sqlx::query!("DELETE FROM agents WHERE id = $1", id.0)
        .execute(pool)
        .await?;
    if result.rows_affected() == 0 {
        return Err(DbError::NotFound);

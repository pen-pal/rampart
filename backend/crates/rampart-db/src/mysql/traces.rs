...
/// NULL` span via a correlated subquery (no LATERAL); `services` is
/// `GROUP_CONCAT(DISTINCT …)` split on `,`. `COUNT`/`CAST AS DOUBLE` keep the
/// aggregates `i64`/`f64` (a bare `SUM` would decode as `DECIMAL`).
// ponytail: GROUP_CONCAT truncates at group_concat_max_len (default 1024) — a
// trace spanning >~50 distinct services would lose the tail of `services`; lift
// the session var if that ever matters.
    ...
    let rows = sqlx::query(
        "SELECT t.* FROM (
           SELECT
             ...
             CAST((MAX(s.end_ns) - MIN(s.start_ns)) / 1000000.0 AS DOUBLE) AS duration_ms,
             COUNT(*) AS span_count,
             COUNT(CASE WHEN s.status_code = 2 THEN 1 END) AS error_count,
           ...
           HAVING (? IS NULL OR MAX(CASE WHEN s.service_name = ? THEN 1 ELSE 0 END) = 1)
              AND (? IS NULL
                   OR CAST((MAX(s.end_ns) - MIN(s.start_ns)) / 1000000.0 AS DOUBLE) >= ?)
              AND (? = 0 OR COUNT(CASE WHEN s.status_code = 2 THEN 1 END) > 0)
         ) t
         WHERE (? IS NULL
                OR t.root_name LIKE CONCAT('%', ?, '%')
                OR t.root_service LIKE CONCAT('%', ?, '%')
                OR t.trace_id LIKE CONCAT('%', ?, '%'))
           AND (? IS NULL

//! MySQL `orgs` domain — organizations + membership, the identity/tenancy core.
//! Mirrors the PG/SQLite `orgs` free-fn surface. MySQL has no `RETURNING`, so
//! inserts use the app-side UUID PK + an INSERT-then-SELECT; the upsert is
//! `ON DUPLICATE KEY UPDATE` (the PG `ON CONFLICT` / SQLite `excluded` form).
...
pub async fn create(pool: &MySqlPool, slug: &str, name: &str) -> DbResult<Org> {
    let id = OrgId::new();
...
pub async fn get(pool: &MySqlPool, id: OrgId) -> DbResult<Org> {
...
pub async fn get_by_slug(pool: &MySqlPool, slug: &str) -> DbResult<Org> {
    let row: Option<OrgRow> =
...
pub async fn update(pool: &MySqlPool, id: OrgId, name: &str) -> DbResult<Org> {
    // Existence check first: MySQL UPDATE reports *changed* rows, so a no-op
    // rename (same name) would look like NotFound. Gate on a SELECT instead.
    get(pool, id).await?;
...
pub async fn list_for_user(pool: &MySqlPool, user_id: UserId) -> DbResult<Vec<Org>> {
...
pub async fn list_members(pool: &MySqlPool, org_id: OrgId) -> DbResult<Vec<OrgMember>> {
...
pub async fn remove_member(pool: &MySqlPool, org_id: OrgId, user_id: UserId) -> DbResult<bool> {
...
pub async fn count_admins(pool: &MySqlPool, org_id: OrgId) -> DbResult<i64> {
    let (n,): (i64,) =
...
mod tests {
    ...
    // seed a minimal user row directly (the MySQL `users` domain lands next slice).
    async fn seed_user(pool: &MySqlPool, email: &str) -> UserId {
    ...
    async fn create_get_by_slug_and_rename(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn membership_upsert_role_and_lists(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn remove_member_and_count_admins(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn create_with_owner_makes_admin(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn default_org_seeded(pool: MySqlPool) {
        let def =

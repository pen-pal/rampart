//! MySQL `slos` domain — service level objectives + budget evaluation. Mirrors
//! the PG/SQLite free-fn surface: list / list_all / get / get_unscoped / create
...
//! ratios over the ported metric_samples. MySQL deltas: `COUNT/SUM FILTER` →
//! `SUM(CASE…)` (CAST SIGNED where decoded i64); `date_bin` → integer bucket
//! `(ts - since) DIV step`; jsonb `labels @> matcher` → one bound
...
//! matching SQLite). `CAST(… AS REAL)` isn't valid MySQL → `* 1e0` forces DOUBLE.
//! `update` drops the `rows_affected()` gate (changed-vs-matched); the trailing
//! `get()` surfaces NotFound.
...
fn slo_from(r: &sqlx::mysql::MySqlRow) -> Slo {
    Slo {
        ...
        window_days: r.get::<i32, _>("window_days"),
...
pub async fn list(pool: &MySqlPool, org_id: OrgId) -> DbResult<Vec<Slo>> {
...
pub async fn list_all(pool: &MySqlPool) -> DbResult<Vec<Slo>> {
...
pub async fn get(pool: &MySqlPool, id: SloId, org_id: OrgId) -> DbResult<Slo> {
...
pub async fn get_unscoped(pool: &MySqlPool, id: SloId) -> DbResult<Slo> {
...
pub async fn create(pool: &MySqlPool, input: NewSlo, org_id: OrgId) -> DbResult<Slo> {
    let id = SloId::new();
...
pub async fn update(pool: &MySqlPool, id: SloId, patch: UpdateSlo, org_id: OrgId) -> DbResult<Slo> {
...
pub async fn delete(pool: &MySqlPool, id: SloId, org_id: OrgId) -> DbResult<()> {
...
/// `JSON_UNQUOTE(JSON_EXTRACT(labels, ?)) = ?` per matcher key. JSON_EXTRACT
/// returns the quoted scalar so it's unquoted; `COLLATE utf8mb4_bin` makes the
/// compare exact (matching SQLite). Empty matcher → no constraint.
fn containment(matcher: &serde_json::Value) -> (String, Vec<String>) {
    let mut sql = String::new();
    let mut binds = Vec::new();
    if let Some(obj) = matcher.as_object() {
        for (k, v) in obj {
            sql.push_str(" AND JSON_UNQUOTE(JSON_EXTRACT(labels, ?)) = ? COLLATE utf8mb4_bin");
...
async fn monitor_ratio(pool: &MySqlPool, monitor: MonitorId, since: i64) -> DbResult<Option<f64>> {
...
async fn metric_ratio(
    ...
    // value is DOUBLE → SUM(DOUBLE) stays DOUBLE (decodes f64; no CAST needed).
    let sql = format!(
        "SELECT COALESCE(SUM(CASE WHEN name = ? THEN value ELSE 0 END), 0) AS good_v,
                COALESCE(SUM(CASE WHEN name = ? THEN value ELSE 0 END), 0) AS total_v
         FROM metric_samples
    ...
    let total_v = row.get::<f64, _>("total_v");
    if total_v <= 0.0 {
        return Ok(None);
    }
    Ok(Some(
        (row.get::<f64, _>("good_v") / total_v * 100.0).clamp(0.0, 100.0),
    ))
}

async fn achieved(pool: &MySqlPool, slo: &Slo, window_seconds: i64) -> DbResult<Option<f64>> {
...
pub async fn compute(pool: &MySqlPool, slo: &Slo) -> DbResult<SloSnapshot> {
...
/// Achieved-ratio trend bucketed into ~`buckets` points (oldest→newest).
/// `date_bin` → integer bucket `(ts - since) DIV step`; `* 1e0` forces DOUBLE.
pub async fn trend(pool: &MySqlPool, slo: &Slo, buckets: i64) -> DbResult<Vec<f64>> {
    ...
    match slo.sli_kind {
        SliKind::Monitor => {
            ...
            let sql = format!(
                "SELECT SUM(CASE WHEN status = 'up' THEN 1 ELSE 0 END) * 1e0
                          / NULLIF(COUNT(*), 0) * 100.0 AS pct
                 FROM heartbeats WHERE monitor_id = ? AND ts >= {since}
                 GROUP BY (ts - {since}) DIV {step} ORDER BY (ts - {since}) DIV {step}"
            );
            let rows = sqlx::query(sqlx::AssertSqlSafe(sql))
                .bind(m.0.to_string())
                .fetch_all(pool)
...
pub async fn evaluate_tick(pool: &MySqlPool) -> DbResult<Vec<SloEvent>> {
...
mod tests {
    use super::*;
    use crate::mysql::{heartbeats, metric_samples, monitors};
    ...
    async fn monitor_slo_crud_and_compute(pool: MySqlPool) {
        let org = super::super::oid(DEF);
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn metric_slo_containment(pool: MySqlPool) {
        let org = super::super::oid(DEF);
        let s = create(
            &pool,
            NewSlo {

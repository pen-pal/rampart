//! MySQL `profiles` domain — continuous-profiling storage. Ported from PG.
//! Mirrors the free-fn surface: insert / list / folded_in_window / fetch_folded
//! / services / profile_types / prune.
//!
//! Dialect: BIGSERIAL→BIGINT AUTO_INCREMENT (no `RETURNING id` → `LAST_INSERT_ID()`),
//! ts→BIGINT unix-seconds (`now() - make_interval(hours=>?)` → Rust-computed
//! cutoff), JSONB→LONGTEXT (serde round-trip), BYTEA→LONGBLOB. Optional
//! service/type filters use the `(? IS NULL OR col = ?)` bind-twice form.
...
pub async fn insert(pool: &MySqlPool, p: NewProfile<'_>, org_id: OrgId) -> DbResult<i64> {
    let res = sqlx::query(
...
fn meta_from(r: &sqlx::mysql::MySqlRow) -> ProfileMeta {
    ProfileMeta {
        id: r.get::<i64, _>("id"),
...
pub async fn services(pool: &MySqlPool, org_id: OrgId) -> DbResult<Vec<String>> {
    let rows = sqlx::query(
...
mod tests {
    ...
    async fn insert_list_window_pickers_prune(pool: MySqlPool) {

//! MySQL `users` domain — accounts, RBAC role, TOTP/2FA state, prefs, GDPR
//! anonymize. Mirrors the PG/SQLite surface. MySQL deltas: no `RETURNING`
//! (insert-then-get); `unixepoch()` → `UNIX_TIMESTAMP()`; `||` → `CONCAT`;
//! upsert → `ON DUPLICATE KEY UPDATE`; and the lockout UPDATE orders the
//! `totp_locked_until` CASE BEFORE the increment because MySQL evaluates SET
//! left-to-right against already-updated column values (unlike the SQL-standard
//! all-old-values behaviour PG/SQLite use).
...
pub async fn count(pool: &MySqlPool) -> DbResult<i64> {
...
/// Insert a user + seed its Default-org membership atomically. Email collision
/// → `Conflict`.
pub async fn create(pool: &MySqlPool, input: NewUser) -> DbResult<User> {
    let id = Uuid::now_v7();
    ...
    .await?;
    tx.commit().await?;
    get(pool, UserId::from_uuid(id)).await
}

pub async fn get_by_email(pool: &MySqlPool, email: &str) -> DbResult<UserWithHash> {
...
pub async fn by_email(pool: &MySqlPool, email: &str) -> DbResult<Option<User>> {
...
pub async fn get(pool: &MySqlPool, id: UserId) -> DbResult<User> {
    let row: Option<UserRow> = sqlx::query_as(
        "SELECT id, email, name, is_admin, role, created_at, last_login_at, totp_enabled
         FROM users WHERE id = ?",
    )
...
pub async fn list(pool: &MySqlPool) -> DbResult<Vec<User>> {
...
pub async fn set_totp_secret(pool: &MySqlPool, id: UserId, secret: &str) -> DbResult<()> {
...
pub async fn enable_totp(pool: &MySqlPool, id: UserId) -> DbResult<()> {
...
pub async fn disable_totp(pool: &MySqlPool, id: UserId) -> DbResult<()> {
...
pub async fn mark_login(pool: &MySqlPool, id: UserId) -> DbResult<()> {
    sqlx::query("UPDATE users SET last_login_at = UNIX_TIMESTAMP() WHERE id = ?")
        .bind(id.0.to_string())
        .execute(pool)
        .await?;
    Ok(())
...
/// Returns whether the account is now locked. The lock CASE is ordered BEFORE
/// the increment so it reads the pre-increment count (MySQL SET is left-to-right
/// over updated values), then a SELECT reports the lock state (no RETURNING).
pub async fn record_totp_failure(
    ...
    sqlx::query(
        "UPDATE users
         SET totp_locked_until = CASE
                 WHEN totp_failed_attempts + 1 >= ? THEN UNIX_TIMESTAMP() + (? * 60)
    ...
    let (locked,): (i64,) = sqlx::query_as(
        "SELECT (totp_locked_until IS NOT NULL AND totp_locked_until > UNIX_TIMESTAMP())
         FROM users WHERE id = ?",
    )
    .bind(id.0.to_string())
    .fetch_one(pool)
...
pub async fn reset_totp_failures(pool: &MySqlPool, id: UserId) -> DbResult<()> {
...
pub async fn set_admin(pool: &MySqlPool, id: UserId, is_admin: bool) -> DbResult<()> {
    let role = if is_admin { Role::Admin } else { Role::Editor };
    set_role_inner(pool, id, role, is_admin).await
}

pub async fn set_role(pool: &MySqlPool, id: UserId, role: Role) -> DbResult<()> {
    set_role_inner(pool, id, role, role.is_admin()).await
}

/// Update the user + mirror the role onto the Default-org membership in one tx,
/// then revoke the target's sessions (privilege change takes effect now).
async fn set_role_inner(pool: &MySqlPool, id: UserId, role: Role, is_admin: bool) -> DbResult<()> {
    let mut tx = pool.begin().await?;
    // Existence check first (MySQL UPDATE reports changed not matched rows).
...
pub async fn delete(pool: &MySqlPool, id: UserId) -> DbResult<()> {
...
pub async fn anonymize(pool: &MySqlPool, id: UserId) -> DbResult<()> {
    let r = sqlx::query(
        "UPDATE users SET
            email = CONCAT('erased-', ?, '@deleted.invalid'),
            name = NULL,
...
pub async fn get_prefs(pool: &MySqlPool, id: UserId) -> DbResult<serde_json::Value> {
...
pub async fn set_prefs(pool: &MySqlPool, id: UserId, prefs: &serde_json::Value) -> DbResult<()> {
...
pub async fn set_password(pool: &MySqlPool, id: UserId, hash: &str) -> DbResult<()> {
...
mod tests {
    ...
    async fn create_seeds_default_membership_and_reads_back(pool: MySqlPool) {
        ...
        // utf8mb4 CI collation: lookup is case-insensitive.
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn set_role_mirrors_membership_and_password(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn totp_lockout_lifecycle(pool: MySqlPool) {
    ...
    #[sqlx::test(migrations = "../../migrations-mysql")]
    async fn prefs_list_anonymize_delete(pool: MySqlPool) {

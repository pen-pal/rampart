//! MySQL `scheduled_reports` domain — periodic uptime digests. Mirrors the
//! PG/SQLite surface. MySQL deltas: insert/update-then-get (no RETURNING); the
//! `due` cadence CASE uses `DATE_FORMAT(FROM_UNIXTIME(...), '%Y-%m')` for the
//! monthly bucket; `unixepoch()`→`UNIX_TIMESTAMP()`. `render` reuses the ported
//! `mysql::monitors::list_all` + `mysql::heartbeats::uptime_pct` and the
//! dialect-neutral `cadence_window_seconds`.
...
fn report_from(r: &sqlx::mysql::MySqlRow) -> ScheduledReport {
    ScheduledReport {
...
pub async fn list(pool: &MySqlPool, org_id: OrgId) -> DbResult<Vec<ScheduledReport>> {
    let sql =
...
pub async fn create(
    ...
    .execute(pool)
    .await?;
    get(pool, ScheduledReportId::from_uuid(id), org_id).await
}
...
pub async fn delete(pool: &MySqlPool, id: ScheduledReportId, org_id: OrgId) -> DbResult<()> {
        ...
        "SELECT {COLS}, org_id FROM scheduled_reports
         WHERE last_sent_at IS NULL
            OR CASE cadence
                 ...
                 WHEN 'monthly' THEN DATE_FORMAT(FROM_UNIXTIME(last_sent_at), '%Y-%m')
                                     < DATE_FORMAT(FROM_UNIXTIME(?), '%Y-%m')
...
pub async fn mark_sent(pool: &MySqlPool, id: ScheduledReportId) -> DbResult<()> {
    sqlx::query("UPDATE scheduled_reports SET last_sent_at = UNIX_TIMESTAMP() WHERE id = ?")
        .bind(id.0.to_string())
        .execute(pool)
        .await?;
    Ok(())
...
mod tests {
    ...
    async fn crud_due_and_mark_sent(pool: MySqlPool) {
        ...
        assert_eq!(due(&pool, now).await.unwrap().len(), 1); // never-sent → due

        mark_sent(&pool, r.id).await.unwrap();
        assert!(get(&pool, r.id, org).await.unwrap().last_sent_at.is_some());
        assert!(due(&pool, now).await.unwrap().is_empty()); // within weekly window
        let later = now + time::Duration::days(8);
        assert_eq!(due(&pool, later).await.unwrap().len(), 1); // due again 8d later

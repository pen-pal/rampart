//! SQLite `digest_buffer` domain — durable backing store for the notifier's
//! per-channel digest buffer. Mirrors the PG free-fn surface (enqueue /
//! drain_due / take_for_channel / delete_by_ids), reusing
//! `crate::digest_buffer::{BufferedEvent, DueChannel}` so the Store trait stays
//! backend-agnostic.
//!
//! Dialect: uuid→TEXT, jsonb event_json→TEXT(JSON), enqueued_at→INTEGER
//! unix-seconds. `drain_due` joins `notifications` so each channel is gated by
//! its own `digest_window_secs`, identical to PG.
...
pub async fn drain_due(pool: &SqlitePool, now: OffsetDateTime) -> DbResult<Vec<DueChannel>> {
    let rows = sqlx::query(
...
pub async fn delete_by_ids(pool: &SqlitePool, ids: &[Uuid]) -> DbResult<()> {
    if ids.is_empty() {
        return Ok(());
    }
    let sql = format!(
...
mod tests {
    ...
    async fn enqueue_drain_take_delete(pool: SqlitePool) {

//! SQLite `rum` domain — Real User Monitoring. Ported from PG/MySQL.
//!
//! Dialect: uuid→TEXT, ts→INTEGER unix-seconds, DOUBLE→REAL, `?` placeholders.
//! **The p75 reads differ from PG:** PG uses `percentile_cont(0.75) WITHIN GROUP`
//! as an aggregate, which SQLite lacks. So the breakdown reads fetch the windowed
//! rows and aggregate (group + count + p75 + last_seen) in Rust — identical to
//! the MySQL tier.
//!
//! ponytail: app-side aggregation is bounded by short RUM retention × the
//! windowed read; if beacon volume outgrows that, precompute rollups on ingest.
...
pub async fn insert_event(pool: &SqlitePool, b: &RumBeacon, org_id: OrgId) -> DbResult<()> {
    sqlx::query(
...
pub async fn apps(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<String>> {
...
mod tests {
    ...
    async fn ingest_and_aggregates(pool: SqlitePool) {
        let org = super::super::oid(DEF);
        // 4 beacons on /home (varying lcp), 1 on /about (Firefox + traced).

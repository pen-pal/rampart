//! SQLite `monitor_groups` domain — the dispatch-path read `any_parent_down`
...
//! it; the routing read that consumes the folder tree lives in `sqlite::routing`.
...
/// parents only (a paused parent doesn't suppress). Mirrors PG.
pub async fn any_parent_down(pool: &SqlitePool, child: MonitorId) -> DbResult<bool> {

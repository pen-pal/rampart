//! SQLite `maintenance` domain — maintenance windows + their monitor sets.
//! Mirrors the Postgres `crate::maintenance` surface for the 12 self-contained
//! functions: list / get / create / update / delete / set_active / attach /
//! detach / is_in_active_window / transitions_needing_notification /
//! mark_notified_start / mark_notified_end.
//!
//! DEFERRED (couple to the not-yet-ported `status_pages` tables —
//! `status_page_subscribers` / `status_page_monitors`):
//! `confirmed_subscriber_emails_for_monitors` and `public_for_status_page`;
//! their `StoreMaintenance` methods stay `unimplemented!()` until status_pages
//! is forked.
//!
//! Dialect: uuid→TEXT, timestamps→INTEGER unix-seconds, bool→INTEGER 0/1,
//! `recurrence` jsonb→TEXT. The recurrence evaluation (`Recurrence::contains`)
//! is pure `rampart_core` logic, reused verbatim. `= ANY($1)` edge hydration →
//! a bound `IN (?,…)` list.
...
fn window_from(r: &sqlx::sqlite::SqliteRow) -> MaintenanceWindow {
    MaintenanceWindow {
...
async fn hydrate_monitors(pool: &SqlitePool, windows: &mut [MaintenanceWindow]) -> DbResult<()> {
    if windows.is_empty() {
        return Ok(());
    }
    let sql = format!(
...
pub async fn list(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<MaintenanceWindow>> {
    let sql =
...
pub async fn create(
    ...
    for m in &input.monitor_ids {
        sqlx::query(
            "INSERT INTO maintenance_window_monitors (window_id, monitor_id) VALUES (?, ?)
...
pub async fn delete(pool: &SqlitePool, id: MaintenanceId, org_id: OrgId) -> DbResult<()> {
...
pub async fn attach(pool: &SqlitePool, window: MaintenanceId, monitor: MonitorId) -> DbResult<()> {
    sqlx::query(
        "INSERT INTO maintenance_window_monitors (window_id, monitor_id) VALUES (?, ?)
         ON CONFLICT DO NOTHING",
    )
...
pub async fn detach(pool: &SqlitePool, window: MaintenanceId, monitor: MonitorId) -> DbResult<()> {
...
pub async fn is_in_active_window(pool: &SqlitePool, monitor: MonitorId) -> DbResult<bool> {
    let rows = sqlx::query(
...
/// notified yet, hydrated with their monitor sets. Mirrors PG (cross-tenant —
/// the scheduler scans every org).
pub async fn transitions_needing_notification(
    ...
    // Hydrate monitor sets in one IN (?,…) round trip.
    let sql = format!(
        "SELECT window_id, monitor_id FROM maintenance_window_monitors WHERE window_id IN ({})",
...
pub async fn mark_notified_start(pool: &SqlitePool, id: MaintenanceId) -> DbResult<()> {
    sqlx::query("UPDATE maintenance_windows SET notified_start_at = unixepoch() WHERE id = ?")
        .bind(id.0.to_string())
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn mark_notified_end(pool: &SqlitePool, id: MaintenanceId) -> DbResult<()> {
    sqlx::query("UPDATE maintenance_windows SET notified_end_at = unixepoch() WHERE id = ?")
        .bind(id.0.to_string())
        .execute(pool)
        .await?;
    Ok(())
...
mod tests {
    ...
    async fn crud_attach_and_active_window(pool: SqlitePool) {
        let org = super::super::oid(DEF);
        let m = a_monitor(&pool).await;
        let now = OffsetDateTime::now_utc();
        // A window live right now (one-shot, contains now).
        let w = create(
            &pool,
            new_window(
                "db upgrade",
        ...
        // monitor is inside the live one-shot window.

//! SQLite `error_tracking` domain — Sentry-lite projects/issues/events. Ported
//! from PG/MySQL (the biggest management-API tail domain).
//!
//! Dialect notes:
//! - uuid→TEXT, ts→INTEGER unix-seconds (`unixepoch()`), JSONB→TEXT (the
//!   affected-users/stats reads use `json_extract`), `release`→quoted.
//! - `ON CONFLICT (project_id,fingerprint) DO NOTHING` claims the fingerprint;
//!   `rows_affected()==1` means we inserted a new issue, else we read + bump the
//!   existing one.
//! - `date_bin` histogram → `(((ts-origin) / step) * step) + origin` (integer
//!   division, like the logs/error histograms).
//! - no FK cascade → `delete` removes events + issues in a tx by hand.
...
fn project_from(r: &sqlx::sqlite::SqliteRow) -> ErrorProject {
    ErrorProject {
...
pub async fn list(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<ErrorProject>> {
    let sql =
...
pub async fn issue_in_org(pool: &SqlitePool, issue: ErrorIssueId, org_id: OrgId) -> DbResult<()> {
...
pub async fn get(pool: &SqlitePool, id: ErrorProjectId) -> DbResult<ErrorProject> {
    get_opt(pool, id).await?.ok_or(DbError::NotFound)
}

pub async fn org_for_project(pool: &SqlitePool, id: ErrorProjectId) -> DbResult<OrgId> {
...
pub async fn get_opt(pool: &SqlitePool, id: ErrorProjectId) -> DbResult<Option<ErrorProject>> {
...
pub async fn update(
    ...
    .execute(pool)
    .await?;
    // Re-select through the org gate (no rows_affected gate — an all-None patch
    // touches no rows but must still return the row, not a false 404).
...
pub async fn delete(pool: &SqlitePool, id: ErrorProjectId, org_id: OrgId) -> DbResult<()> {
...
fn issue_from(r: &sqlx::sqlite::SqliteRow) -> ErrorIssue {
    ErrorIssue {
...
pub async fn record_event(
    ...
    // Claim the fingerprint as a new issue (ON CONFLICT over the unique index).
    ...
    let stacktrace = if ev.frames.is_empty() {
        ...
        serde_json::to_string(&ev.frames).ok()
    };
    let trace_id = ev
        .raw
...
pub async fn get_issue(pool: &SqlitePool, id: ErrorIssueId) -> DbResult<ErrorIssue> {
...
pub async fn issue_affected_users(
    ...
    let rows = sqlx::query(
        "SELECT COALESCE(json_extract(context, '$.user.id'),
                         json_extract(context, '$.user.email'),
                         json_extract(context, '$.user.username')) AS ident,
...
pub async fn issue_stats(pool: &SqlitePool, id: ErrorIssueId) -> DbResult<IssueStats> {
    let (users,): (i64,) = sqlx::query_as(
        "SELECT COUNT(DISTINCT COALESCE(json_extract(context, '$.user.id'),
                                        json_extract(context, '$.user.email'),
                                        json_extract(context, '$.user.username')))
         FROM error_events
         WHERE issue_id = ? AND json_extract(context, '$.user') IS NOT NULL",
    )
    .bind(id.0.to_string())
...
pub async fn assignable_users(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<AssignableUser>> {
...
fn event_from(r: &sqlx::sqlite::SqliteRow) -> ErrorEvent {
    let json = |name: &str| {
...
/// each DELETE's lock footprint on the high-volume `error_events` table. Issues
/// persist. Returns rows removed.
pub async fn prune(pool: &SqlitePool) -> DbResult<u64> {
    const SQL: &str = "DELETE FROM error_events WHERE id IN (\
         ...
         WHERE e.ts < unixepoch() - p.retention_days * 86400 \
         LIMIT ?)";
    let mut total = 0u64;
    loop {
...
mod tests {
    ...
    async fn projects_record_group_stats_prune(pool: SqlitePool) {
        ...
        // assign round-trips.
        let uid = super::super::uid("00000000-0000-0000-0000-0000000000aa");
        let _ = assign_issue(&pool, issue.id, None).await.unwrap();
        let _ = uid; // assignable_users needs org members; covered by mysql/PG.

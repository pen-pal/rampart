...
impl StoreIngestKeys for SqliteStore {
    async fn create_ingest_key(
        ...
        crate::sqlite::ingest_keys::create(&self.pool, org_id, label, kind, allowed_origins).await
    }
    ...
    async fn find_ingest_key_by_token(
        ...
        crate::sqlite::ingest_keys::find_by_token(&self.pool, token).await
    }

    async fn touch_ingest_key_last_used(&self, id: Uuid) -> DbResult<()> {
        crate::sqlite::ingest_keys::touch_last_used(&self.pool, id).await
    }

    async fn list_ingest_keys_for_org(&self, org_id: OrgId) -> DbResult<Vec<IngestKey>> {
        crate::sqlite::ingest_keys::list_for_org(&self.pool, org_id).await
    }

    async fn delete_ingest_key(&self, id: Uuid, org_id: OrgId) -> DbResult<bool> {
        crate::sqlite::ingest_keys::delete(&self.pool, id, org_id).await
    }
}

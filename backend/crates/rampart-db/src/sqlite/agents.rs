//! SQLite `agents` domain — remote probe workers. Mirrors the Postgres
//! `crate::agents` free-fn surface: list / get / create / update / delete /
//! lookup (token resolver) / touch_seen.
//!
//! Tokens are `rmpa_<40 url-safe chars>`; only the SHA-256 hash is stored
//! (unique). `online` is derived from `last_seen_at` against the shared
//! [`ONLINE_GRACE_SECONDS`] grace window at read time, exactly as PG does.
//! Dialect: uuids TEXT, timestamps INTEGER unix-seconds, `monitor_count` via a
//! `LEFT JOIN monitors … GROUP BY`.
...
/// Build an `Agent` from a row that carries the agent columns plus a
/// `monitor_count` aggregate. `last_seen_at` is a nullable INTEGER.
fn agent_from(r: &sqlx::sqlite::SqliteRow) -> Agent {
...
pub async fn list(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<Agent>> {
    let rows = sqlx::query(
...
pub async fn get(pool: &SqlitePool, id: AgentId, org_id: OrgId) -> DbResult<Agent> {
    let row = sqlx::query(
        "SELECT a.id, a.name, a.location, a.version, a.last_seen_at,
                a.created_at, a.org_id, COUNT(m.id) AS monitor_count
         FROM agents a LEFT JOIN monitors m ON m.agent_id = a.id
...
pub async fn create(pool: &SqlitePool, input: NewAgent, org_id: OrgId) -> DbResult<IssuedAgent> {
    ...
    let row = sqlx::query(
        "INSERT INTO agents (id, name, location, token_hash, org_id)
         VALUES (?, ?, ?, ?, ?)
         RETURNING id, name, location, version, last_seen_at, created_at, org_id,
                   0 AS monitor_count",
    )
    .bind(id.0.to_string())
    .bind(input.name)
...
/// Delete an agent and return its monitors to local probing. SQLite doesn't
/// enforce the PG `ON DELETE SET NULL` (monitors.agent_id has no FK here), so
/// clear the column explicitly in one tx.
pub async fn delete(pool: &SqlitePool, id: AgentId, org_id: OrgId) -> DbResult<()> {
...
pub async fn lookup(pool: &SqlitePool, token: &str) -> DbResult<Agent> {
...
/// Bump `last_seen_at` (and the self-reported version when sent) on every
/// agent poll/report. Fire-and-forget on the hot path.
pub async fn touch_seen(pool: &SqlitePool, id: AgentId, version: Option<&str>) -> DbResult<()> {
    sqlx::query(
        "UPDATE agents SET last_seen_at = unixepoch(), version = COALESCE(?, version) WHERE id = ?",
    )
    .bind(version)
    .bind(id.0.to_string())
    .execute(pool)
...
mod tests {
    ...
    async fn a_monitor(pool: &SqlitePool) -> rampart_core::ids::MonitorId {
        let org = oid(DEF);
        monitors::create(
            pool,
            NewMonitor {
    ...
    async fn create_lookup_touch_and_count(pool: SqlitePool) {
        ...
        // Freshly created → offline (last_seen_at NULL); touch_seen flips it.
    ...
    #[sqlx::test(migrations = "../../migrations-sqlite")]
    async fn update_clear_location_and_delete_unassigns(pool: SqlitePool) {
        let org = oid(DEF);

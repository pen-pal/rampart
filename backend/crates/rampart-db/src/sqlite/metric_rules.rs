//! SQLite `metric_rules` domain — threshold/anomaly alert rules over ingested
//! metrics. Mirrors the PG free-fn surface: list / list_all / get /
//! get_unscoped / create / update / delete / evaluate_tick.
//!
//! `evaluate_tick` reuses the pure `rampart_core::metric_rule::rule_transition`
//! state machine and the now-ported `sqlite::metric_samples::latest`/`baseline`
//! reads. Dialect: uuid→TEXT, jsonb labels→TEXT, `UUID[]` channel_ids→JSON
//! array TEXT, double→REAL, ts→INTEGER unix-seconds, `op`/`enabled` as
//! TEXT/INTEGER round-trips.
...
fn rule_from(r: &sqlx::sqlite::SqliteRow) -> MetricRule {
    MetricRule {
...
pub async fn list(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<MetricRule>> {
...
pub async fn list_all(pool: &SqlitePool) -> DbResult<Vec<MetricRule>> {
...
pub async fn get(pool: &SqlitePool, id: MetricRuleId, org_id: OrgId) -> DbResult<MetricRule> {
...
pub async fn get_unscoped(pool: &SqlitePool, id: MetricRuleId) -> DbResult<MetricRule> {
...
pub async fn delete(pool: &SqlitePool, id: MetricRuleId, org_id: OrgId) -> DbResult<()> {
...
/// the sample reads delegate to the ported `sqlite::metric_samples`.
...
mod tests {
    ...
    async fn push_sample(pool: &SqlitePool, value: f64) {
    ...
    async fn crud_and_evaluate_fire_resolve(pool: SqlitePool) {
        let org = super::super::oid(DEF);

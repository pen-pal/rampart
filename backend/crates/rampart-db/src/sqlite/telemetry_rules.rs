//! SQLite `telemetry_rules` domain — threshold alert rules over the telemetry
//! tiers. Mirrors the PG free-fn surface: list / list_all / get / get_unscoped
//! / create / update / delete / evaluate_tick.
//!
//! `evaluate_tick` reuses the pure `rule_transition` state machine. `observe`
//! computes the per-tier aggregate: the **logs + traces** tiers are real
//! (those domains are ported), the **trace_latency** percentile is computed
//! app-side (`p_cont`, no SQLite percentile_cont). The error_tracking, profiles
//! and rum tiers are NOT yet forked to SQLite, so those kinds return `None`
//! (no-data → resolve, never a false fire) until they land — documented so the
//! gap is visible rather than a runtime "no such table".
//!
//! Dialect: uuid→TEXT, UUID[] channel_ids→JSON, ts→INTEGER unix-seconds.
...
fn rule_from(r: &sqlx::sqlite::SqliteRow) -> TelemetryRule {
    TelemetryRule {
        ...
        for_seconds: r.get::<i64, _>("for_seconds") as i32,
...
pub async fn list(pool: &SqlitePool, org_id: OrgId) -> DbResult<Vec<TelemetryRule>> {
    let sql =
...
pub async fn list_all(pool: &SqlitePool) -> DbResult<Vec<TelemetryRule>> {
...
pub async fn get(pool: &SqlitePool, id: TelemetryRuleId, org_id: OrgId) -> DbResult<TelemetryRule> {
...
pub async fn get_unscoped(pool: &SqlitePool, id: TelemetryRuleId) -> DbResult<TelemetryRule> {
...
pub async fn delete(pool: &SqlitePool, id: TelemetryRuleId, org_id: OrgId) -> DbResult<()> {
...
/// tiers are real; error_tracking / profiles / rum tiers aren't forked to SQLite
/// yet → `None` (no-data → resolve, never a false fire).
async fn observe(pool: &SqlitePool, rule: &TelemetryRule) -> DbResult<Option<f64>> {
    ...
    match rule.kind {
        ...
        TelemetryRuleKind::TraceErrorRate => {
            let (total, errs): (i64, i64) = sqlx::query_as(
                "SELECT COUNT(*), COALESCE(SUM(CASE WHEN status_code = 2 THEN 1 ELSE 0 END), 0)
                 FROM spans
                 WHERE received_at >= ? AND (? = '' OR service_name = ?) AND org_id = ?",
            )
            .bind(cutoff)
...
pub async fn evaluate_tick(pool: &SqlitePool) -> DbResult<Vec<RuleEvent>> {
...
mod tests {
    ...
    async fn crud_and_log_volume_fire(pool: SqlitePool) {

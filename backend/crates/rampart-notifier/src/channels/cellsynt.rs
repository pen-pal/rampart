...
pub struct Cellsynt { cfg: CellsyntConfig, client: reqwest::Client }
...
impl Cellsynt {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.username.is_empty() || cfg.password.is_empty() || cfg.destination.is_empty() {
            return Err(ChannelError::BadConfig("username + password + destination required".into()));
        }
...
impl Channel for Cellsynt {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        ...
        let resp = self.client.get("https://se-1.cellsynt.net/sms.php")
            .query(&[

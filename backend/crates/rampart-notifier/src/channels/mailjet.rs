...
pub struct Mailjet { cfg: MailjetConfig, client: reqwest::Client }
...
impl Mailjet {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.api_key.is_empty() || cfg.api_secret.is_empty() {
            return Err(ChannelError::BadConfig("api_key + api_secret required".into()));
        }
...
impl Channel for Mailjet {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let payload = Payload {
            messages: vec![Msg {
                from: Addr { email: &self.cfg.from_email, name: &self.cfg.from_name },
                to:   vec![Addr { email: &self.cfg.to_email, name: &self.cfg.to_name }],
                subject,
                text_part: body,
            }],
        };
        let resp = self.client.post("https://api.mailjet.com/v3.1/send")

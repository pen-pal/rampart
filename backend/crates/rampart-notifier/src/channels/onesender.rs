...
pub struct Onesender { cfg: OnesenderConfig, client: reqwest::Client }
...
impl Onesender {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.api_token.is_empty() || cfg.recipient.is_empty() {
            return Err(ChannelError::BadConfig("api_token + recipient required".into()));
        }
...
impl Channel for Onesender {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/api/v1/send-message", self.cfg.base_url.trim_end_matches('/'));
        let payload = Payload {
            recipient: &self.cfg.recipient,
            kind:      "text",
            message:   Msg { text: format!("{subject}\n{body}") },
        };

...
pub struct Newrelic { cfg: NewrelicConfig, client: reqwest::Client }
...
impl Newrelic {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.insert_key.is_empty() || cfg.account_id.is_empty() {
            return Err(ChannelError::BadConfig("insert_key + account_id required".into()));
        }
...
impl Channel for Newrelic {
    async fn send(&self, subject: &str, body: &str, event: &Event) -> Result<(), ChannelError> {
        let host = if self.cfg.region == "eu" { "https://insights-collector.eu01.nr-data.net" } else { "https://insights-collector.newrelic.com" };

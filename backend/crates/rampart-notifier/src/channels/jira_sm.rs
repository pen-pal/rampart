...
fn default_issue_type() -> String { "Incident".into() }

pub struct JiraSm { cfg: JiraSmConfig, client: reqwest::Client }
...
impl Channel for JiraSm {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/rest/api/3/issue", self.cfg.site_url.trim_end_matches('/'));
        let payload = Payload {
            fields: Fields {
                project: Named { key: Some(&self.cfg.project_key), name: None },
                summary: subject,
                description: Doc {
                    kind: "doc",
                    version: 1,
                    content: vec![Para {
                        kind: "paragraph",
                        content: vec![Text { kind: "text", text: body }],
                    }],
                },
                issuetype: Named { key: None, name: Some(&self.cfg.issue_type) },
            },
        };

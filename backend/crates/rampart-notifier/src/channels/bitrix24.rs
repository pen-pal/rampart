...
pub struct Bitrix24 { cfg: Bitrix24Config, client: reqwest::Client }
...
impl Bitrix24 {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if !cfg.webhook_url.starts_with("http") || cfg.user_id.trim().is_empty() {
            return Err(ChannelError::BadConfig("webhook_url and user_id required".into()));
        }
...
impl Channel for Bitrix24 {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/im.notify.system.add.json",

...
fn default_base() -> String { "https://api.sparkpost.com".into() }

pub struct Sparkpost { cfg: SparkpostConfig, client: reqwest::Client }
...
#[derive(Serialize)]
struct Options { sandbox: bool }
#[derive(Serialize)]
struct Content<'a> { from: &'a str, subject: &'a str, text: &'a str }
#[derive(Serialize)]
struct Rcpt<'a> { address: Addr<'a> }
#[derive(Serialize)]
...
impl Channel for Sparkpost {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let payload = Payload {
            ...
            content: Content { from: &self.cfg.from, subject, text: body },
            recipients: self.cfg.to.split(',').map(str::trim).filter(|s| !s.is_empty())
                .map(|e| Rcpt { address: Addr { email: e } }).collect(),
        };
        let url = format!("{}/api/v1/transmissions", self.cfg.base_url.trim_end_matches('/'));

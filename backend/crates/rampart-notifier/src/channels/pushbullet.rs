...
pub struct Pushbullet { cfg: PushbulletConfig, client: reqwest::Client }
...
impl Channel for Pushbullet {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let payload = Payload { kind: "note", title: subject, body, device: &self.cfg.device_iden };
        let resp = self.client.post("https://api.pushbullet.com/v2/pushes")

...
pub struct GoogleSheets { cfg: GoogleSheetsConfig, client: reqwest::Client }
...
impl Channel for GoogleSheets {
    async fn send(&self, subject: &str, body: &str, event: &Event) -> Result<(), ChannelError> {
        let payload = Payload {
            timestamp: event.heartbeat.ts.format(&time::format_description::well_known::Rfc3339).unwrap_or_default(),
            title: subject,
            body,

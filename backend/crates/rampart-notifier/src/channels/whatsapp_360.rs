...
pub struct Whatsapp360 { cfg: W360Config, client: reqwest::Client }
...
impl Channel for Whatsapp360 {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let payload = Payload { phone: &self.cfg.phone, message: format!("{subject}\n{body}") };
        let resp = self.client.post("https://api.360messenger.com/sendMessage")

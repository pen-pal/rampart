...
pub struct SmsEagle { cfg: SmsEagleConfig, client: reqwest::Client }
...
impl SmsEagle {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if !cfg.base_url.starts_with("http") || cfg.access_token.is_empty() {
            return Err(ChannelError::BadConfig("base_url + access_token required".into()));
        }
...
impl Channel for SmsEagle {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/api/v2/messages/sms", self.cfg.base_url.trim_end_matches('/'));
        let payload = Payload {

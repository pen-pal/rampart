...
pub struct Clicksend { cfg: ClicksendConfig, client: reqwest::Client }
...
impl Channel for Clicksend {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        ...
        let payload = Payload {
            messages: self.cfg.to.split(',').map(str::trim).filter(|s| !s.is_empty())
                .map(|num| Sms {
                    source: "rampart",
                    from: &self.cfg.from,
                    to: num,
        ...
        let resp = self.client.post("https://rest.clicksend.com/v3/sms/send")

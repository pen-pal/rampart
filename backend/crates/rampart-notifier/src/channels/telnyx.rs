...
pub struct Telnyx { cfg: TelnyxConfig, client: reqwest::Client }
...
impl Channel for Telnyx {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        ...
        for to in self.cfg.to.split(',').map(str::trim).filter(|s| !s.is_empty()) {
            let payload = Payload { from: &self.cfg.from, to, text: text.clone() };
            let resp = self.client.post("https://api.telnyx.com/v2/messages")

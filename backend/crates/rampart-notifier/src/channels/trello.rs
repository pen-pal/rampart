...
pub struct Trello { cfg: TrelloConfig, client: reqwest::Client }
...
impl Trello {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.key.is_empty() || cfg.token.is_empty() || cfg.list_id.is_empty() {
            return Err(ChannelError::BadConfig("key + token + list_id required".into()));
        }
...
impl Channel for Trello {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let resp = self.client.post("https://api.trello.com/1/cards")
            .query(&[

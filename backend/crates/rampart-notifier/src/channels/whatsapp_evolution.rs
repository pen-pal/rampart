...
pub struct WhatsappEvolution { cfg: EvolutionConfig, client: reqwest::Client }
...
impl WhatsappEvolution {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.api_key.is_empty() || cfg.instance.is_empty() || cfg.number.is_empty() {
            return Err(ChannelError::BadConfig("api_key + instance + number required".into()));
        }
...
impl Channel for WhatsappEvolution {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/message/sendText/{}",
            self.cfg.base_url.trim_end_matches('/'),
            self.cfg.instance,
        );
        let payload = Payload { number: &self.cfg.number, text: format!("{subject}\n{body}") };

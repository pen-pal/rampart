...
pub struct Smsc { cfg: SmscConfig, client: reqwest::Client }
...
impl Smsc {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.login.is_empty() || cfg.psw.is_empty() || cfg.phones.is_empty() {
            return Err(ChannelError::BadConfig("login + psw + phones required".into()));
        }
...
impl Channel for Smsc {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        ...
        let resp = self.client.get("https://smsc.ru/sys/send.php")
            .query(&[

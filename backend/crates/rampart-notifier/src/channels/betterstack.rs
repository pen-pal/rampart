...
pub struct Betterstack { cfg: BetterstackConfig, client: reqwest::Client }
...
impl Channel for Betterstack {
    async fn send(&self, subject: &str, body: &str, event: &Event) -> Result<(), ChannelError> {
        let severity = if event.heartbeat.status == MonitorStatus::Up { "info" } else { "critical" };
        let payload = Payload {
            summary: subject, description: body, severity,

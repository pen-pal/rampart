...
pub struct HomeAssistant { cfg: HomeAssistantConfig, client: reqwest::Client }
...
impl HomeAssistant {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.long_lived_token.trim().is_empty() || cfg.notify_service.trim().is_empty() {
            return Err(ChannelError::BadConfig("long_lived_token and notify_service required".into()));
        }
...
impl Channel for HomeAssistant {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let url = format!("{}/api/services/notify/{}",
            self.cfg.base_url.trim_end_matches('/'),
            self.cfg.notify_service,
        );
        let resp = self.client.post(&url)
            .bearer_auth(&self.cfg.long_lived_token)
            .json(&Payload { title: subject, message: body })
            .send().await?;

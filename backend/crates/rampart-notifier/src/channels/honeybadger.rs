...
pub struct Honeybadger { cfg: HoneybadgerConfig, client: reqwest::Client }
...
#[derive(Serialize)]
struct Notifier<'a> { name: &'static str, version: &'static str, url: &'a str }
#[derive(Serialize)]
struct HbError<'a> { class: &'static str, message: String, tags: Vec<&'a str> }
#[derive(Serialize)]
struct Server<'a> { environment_name: &'a str }

#[async_trait]
impl Channel for Honeybadger {
    async fn send(&self, subject: &str, body: &str, event: &Event) -> Result<(), ChannelError> {
        let payload = Payload {
            notifier: Notifier { name: "rampart", version: "0.1", url: "https://github.com/rampart-io/rampart" },
            error: HbError {
                class: "MonitorAlert",
                message: format!("{subject}\n{body}"),
                tags: vec![&event.monitor.name],
            },
            server: Server { environment_name: &self.cfg.environment },
        };
        let resp = self.client.post("https://api.honeybadger.io/v1/notices")

...
pub struct Webex { cfg: WebexConfig, client: reqwest::Client }
...
impl Webex {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        ...
        if cfg.bot_token.is_empty() || cfg.room_id.is_empty() {
            return Err(ChannelError::BadConfig("bot_token + room_id required".into()));
        }
...
impl Channel for Webex {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        let payload = Payload { room_id: &self.cfg.room_id, markdown: format!("**{subject}**\n{body}") };
        let resp = self.client.post("https://webexapis.com/v1/messages")

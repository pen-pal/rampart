...
fn default_kind() -> String { "stream".into() }

pub struct Zulip { cfg: ZulipConfig, client: reqwest::Client }

impl Zulip {
    pub fn from_config(raw: &serde_json::Value) -> Result<Self, ChannelError> {
        let cfg: ZulipConfig = serde_json::from_value(raw.clone())
            .map_err(|e| ChannelError::BadConfig(e.to_string()))?;
        if cfg.server.trim().is_empty() || cfg.bot_email.trim().is_empty() || cfg.bot_key.trim().is_empty() {
            return Err(ChannelError::BadConfig("server + bot_email + bot_key required".into()));
        }
...
impl Channel for Zulip {
    async fn send(&self, subject: &str, body: &str, _event: &Event) -> Result<(), ChannelError> {
        ...
        if self.cfg.kind == "stream" {
            form.push(("topic", self.cfg.topic.clone().unwrap_or_else(|| "rampart".into())));
        }

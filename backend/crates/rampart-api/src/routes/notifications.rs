...
use rampart_db::notifications::{MonitorChannelCount, NewNotification, Notification, UpdateNotification};
...
async fn get_one(
    ...
    Ok(Json(rampart_db::notifications::get(state.pool(), id).await?))
}
...
async fn update(
    ...
    let id = parse_notif(&id)?;
    Ok(Json(rampart_db::notifications::update(state.pool(), id, input).await?))
}
...
async fn counts(
    ...
    Ok(Json(rampart_db::notifications::counts_per_monitor(state.pool()).await?))
}
...
async fn list_for_monitor(
    ...
    Ok(Json(rampart_db::notifications::for_monitor(state.pool(), mid).await?))
}

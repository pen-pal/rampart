//! Integration test for the Site24x7 importer's parse + map pass.
//!
...
const FIXTURE: &str = include_str!("fixtures/site24x7-sample.json");
...
fn fixture_parses_and_maps_expected_kinds() {
    let plan = site24x7::parse_and_map(FIXTURE).expect("fixture parses");

    // 5 entries total in the fixture; one is REALBROWSER which has no
    ...
    assert_eq!(plan.skipped[0].source_kind, "REALBROWSER");

    assert_eq!(count(&plan, MonitorKind::Http), 1);
    assert_eq!(count(&plan, MonitorKind::Keyword), 1);
    assert_eq!(count(&plan, MonitorKind::Postgres), 1);
...
fn fixture_field_translation_is_correct() {
    ...
    let pg = plan
        .mapped
        .iter()
        .find(|m| m.mapped_kind == MonitorKind::Postgres)
        .expect("postgres entry present");
    assert_eq!(pg.new_monitor.name, "Acme primary DB");
    assert_eq!(
        pg.new_monitor.hostname.as_deref(),
        Some("db.internal.example.com")
    );
    assert_eq!(pg.new_monitor.port, Some(5432));
    assert_eq!(pg.new_monitor.interval_seconds, 300);
    assert_eq!(pg.new_monitor.timeout_seconds, 15);
    assert_eq!(pg.new_monitor.max_retries, 2);

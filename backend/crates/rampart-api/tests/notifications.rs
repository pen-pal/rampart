...
async fn patch_renames_and_toggles_active(pool: PgPool) {
    ...
    let updated: Value = json(
        &r,
        Method::PATCH,
        &format!("/v1/notifications/{id}"),
        Some(json!({ "name": "renamed", "active": false })),
        Some(&c),
    )
    .await;
...
async fn attach_detach_via_api(pool: PgPool) {
    ...
    let (s1, _, _) = request(
        &r,
        Method::POST,
        &format!("/v1/monitors/{mid}/notifications/{nid}"),
        None,
        Some(&c),
    )
    .await;
    ...
    let attached: Vec<Value> = json(
        &r,
        Method::GET,
        &format!("/v1/monitors/{mid}/notifications"),
        None,
        Some(&c),
    )
    .await;
...
async fn template_assign_and_clear(pool: PgPool) {
    ...
    let updated: Value = json(
        &r,
        Method::PATCH,
        &format!("/v1/notifications/{nid}"),
        Some(json!({ "template_id": tid })),
        Some(&c),
    )
    .await;

...
async fn list_empty_by_default(pool: PgPool) {
    ...
    let ts: Vec<Value> = json(&r, Method::GET, "/v1/notification-templates", None, Some(&c)).await;
    assert!(ts.is_empty());
}
...
async fn create_round_trips_all_fields(pool: PgPool) {
    ...
    let t: Value = json(&r, Method::POST, "/v1/notification-templates",
        Some(payload("Concise")), Some(&c)).await;
    assert_eq!(t["name"], "Concise");
    assert_eq!(t["event_kind"], "monitor_down");
    assert_eq!(t["channel_kinds"], json!(["slack", "webhook"]));

    let id = t["id"].as_str().unwrap();
    let g: Value = json(&r, Method::GET, &format!("/v1/notification-templates/{id}"), None, Some(&c)).await;
...
async fn patch_changes_body(pool: PgPool) {
    ...
    let c = register_admin(&r).await;
    let t: Value = json(&r, Method::POST, "/v1/notification-templates",
        Some(payload("Patched")), Some(&c)).await;
    let id = t["id"].as_str().unwrap();
    let p: Value = json(&r, Method::PATCH, &format!("/v1/notification-templates/{id}"),
        Some(json!({ "body_template": "new body" })), Some(&c)).await;
...
async fn delete_round_trip(pool: PgPool) {
    ...
    let c = register_admin(&r).await;
    let t: Value = json(&r, Method::POST, "/v1/notification-templates",
        Some(payload("Toast")), Some(&c)).await;
    let id = t["id"].as_str().unwrap();
    let (s, _, _) = request(&r, Method::DELETE,
        &format!("/v1/notification-templates/{id}"), None, Some(&c)).await;
    assert_eq!(s, StatusCode::NO_CONTENT);
    let (s2, _, _) = request(&r, Method::GET,
        &format!("/v1/notification-templates/{id}"), None, Some(&c)).await;
...
async fn duplicate_name_returns_409(pool: PgPool) {
    let r = common::router(pool);
    let c = register_admin(&r).await;
    json::<Value>(&r, Method::POST, "/v1/notification-templates",
        Some(payload("DupName")), Some(&c)).await;
    let (s, _, _) = request(&r, Method::POST, "/v1/notification-templates",
        Some(payload("DupName")), Some(&c)).await;
